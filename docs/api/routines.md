...
If a check fails, the routine run is finalized as `skipped`; `triggerPayload.paperclipActionabilityPreflight` records the deterministic state, blocker class, owner, fingerprint, duplicate count, self-heal decision, and any standing blocker issue id. Missing credentials, provider capacity, workspace cleanup, and human-owned blockers create or reuse one `factory_guard` issue. Hard repeated agent/human blockers can still pause the routine on the third identical fingerprint, but system-owned cadence and upstream-artifact waits stay active. Scheduled system-owned waits self-heal by pulling the next run into a one-hour recovery window when the natural next run is farther away, capped at three reschedules per fingerprint; when the cap is exhausted Paperclip creates or reuses one self-healing guard and leaves the routine active for its next natural run.

Portfolio OS control-plane routines derive their upstream artifact hash from canonical Portfolio OS files during unattended-factory configuration. Market and VOC sweeps hash `inputs/market_signals/latest.csv` and `inputs/voc/latest.csv`; the evidence intake gate hashes both files plus `data/frozen_selection.json`; composition and graduation lanes hash the frozen selection and execution state files. This keeps stale guard evidence from blocking a fresh run after valid canonical evidence is recovered.

Portfolio OS Council Chamber routines are intentionally event-generating rather than maintenance coalesced. Their actionability contract carries a Council evidence gate: score hypotheses out of 100 using VOC signal (25), market size and trajectory (20), repo feasibility (20), competitive gap (20), and council confidence (15). A hypothesis promotes into build or go-to-market execution at score `>= 70` only when buyer/user, revenue mechanism, first tangible deliverable, cheapest validation step, and duplicate-hypothesis check are present. Distinct Council hypotheses should become separate child issues immediately; below-threshold hypotheses become research or validation issues, while promoted hypotheses become concrete Paperclip/Hermes execution issues. Scratch output belongs in the Paperclip issue document key `council-hypothesis-ledger` and is mirrored under Portfolio OS `data/council_hypotheses/paperclip/`.
