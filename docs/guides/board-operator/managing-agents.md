...
Hermes agents using OpenCode Go store the bare model id, such as `deepseek-v4-flash`, and pin `adapterConfig.provider` to `opencode-go`. Manual paid model choices such as `qwen3.7-max` and `deepseek-v4-pro` must stay on OpenCode Go rather than inheriting Hermes' global free-model fallback.
