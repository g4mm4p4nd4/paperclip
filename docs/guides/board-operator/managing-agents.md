...
Routine actionability is enforced before issue creation. Provider-capacity backoff, missing credentials, human-owned states, maintenance cadence, unchanged upstream artifact hashes, and dirty release/QA/deploy workspaces finalize the routine run as `skipped` with `paperclipActionabilityPreflight` evidence instead of waking an agent. Board-owned blockers create one reusable `factory_guard` issue; workspace blockers create one cleanup issue. Hard repeated agent/human blockers can still pause the routine on the third identical fingerprint, but system-owned cadence and upstream-artifact waits now keep the routine active. Scheduled system waits self-heal by pulling the next run into a one-hour recovery window when the natural next run is farther away, capped at three reschedules per fingerprint; after that cap, Paperclip creates or reuses one self-healing guard and leaves the routine active.

Portfolio OS Council Chamber routines are central direction engines. They use `always_enqueue` so competing Council hypotheses can become separate child issues instead of disappearing into an existing coalesced run. The Council evidence gate promotes hypotheses at score `>= 70` using VOC signal, market size and trajectory, repo feasibility, competitive gap, and council confidence, with hard gates for buyer/user, revenue mechanism, first tangible deliverable, cheapest validation step, and duplicate-hypothesis check. Council scratch output belongs in the Paperclip issue document key `council-hypothesis-ledger` and is mirrored under Portfolio OS `data/council_hypotheses/paperclip/`.
