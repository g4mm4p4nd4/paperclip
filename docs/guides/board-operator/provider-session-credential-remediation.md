...
- The CLI opens a named maintenance connection and appends `statement_timeout=0` to the PostgreSQL startup options for every pooled connection. The full historical scan can exceed the request-traffic timeout; the exclusive maintenance acknowledgement, zero-client fence, bounded file reads, and database locks bound this maintenance-only exception.
